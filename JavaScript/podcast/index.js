document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderPodcasts() {
        podcastList.innerHTML = podcasts.map((podcast) => {
            ...
            const ratingHtml = podcast.rating === 'New'
                ? `<span class="stat-badge"><i class="fa-solid fa-star-half-stroke"></i> New Show</span>`
                : `<span class="stat-badge"><i class="fa-solid fa-star"></i> ${podcast.rating}</span>`;

            return `
            <div class="podcast-card" onclick="openPodcast('${podcast.rss}','${podcast.appleEmbed}',event)" tabindex="0" role="button" aria-label="Open ${podcast.title}">
                ...
                <div class="podcast-content">
                    ...
                    <div class="podcast-creator ${isThaiText(podcast.creator) ? 'thai-font-p' : 'english-font-p'}">by ${podcast.creator}</div>
    ...
    function fetchRSS(feedUrl, proxyIndex = 0) {
        if (proxyIndex >= CORS_PROXIES.length) {
            return Promise.reject(new Error('Unable to load feed. Please try again later.'));
        }
    ...
    function loadFeed(feedUrl) {
        ...
        fetchRSS(feedUrl)
            .then(data => {
                ...
                if (items.length === 0) {
                    episodeListContainer.innerHTML = '<div style="text-align:center;padding:3rem;color:var(--secondary-text);">No episodes found.</div>';
                } else {
                    episodeListContainer.innerHTML = '';
                    items.forEach((item) => {
                        ...
                        card.innerHTML = `
                            ...
                            <div class="ep-meta">
                                <div class="play-btn-small">
                                    ${isActive && isPlaying
                                ? '<i class="fa-solid fa-pause"></i> Playing'
                ...
                if (currentAppleEmbed) {
                    ...
                    loadingScreen.innerHTML = `
                        <div style="text-align:center;padding:2rem;">
                            ...
                            Error loading podcast.<br><small style="opacity:0.7;">${err.message}</small>
                        </div>`;
                }
            });
    }
